<table class="table table-hover table-centered table-condensed">
  <thead>
      ...
      <th colspan="4">
        ...
        <span style="color: FF9002;">
          ...
          {% button class="btn btn-xs btn-onnet pull-right"
                    text=_"refresh list"
                    action={emit signal={update_braintree_transactions_list_tpl
    ...
    <tr id="{{ #mybttr }}"
        style="{% if not m.kazoo[{ui_element_opened element="bt_transactions_list_opened"}] %}
                 display: none;
               {% endif %}">
      <th colspan="3"></th>
    </tr>
...
<span id="bt_transactions_list_opened"
      style="{% if not m.kazoo[{ui_element_opened element="bt_transactions_list_opened"}] %}
               display: none;
             {% endif %}">
  {% wire action={connect signal={update_braintree_transactions_list_tpl
                                  signal_filter=m.kazoo.signal_filter
                                 }
