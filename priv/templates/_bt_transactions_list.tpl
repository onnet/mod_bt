  ...
  <tbody>
    {% for transaction in m.kazoo.kz_bt_transactions %}
      <tr id={{ transaction[1]["id"] }}
          ...
          {% endif %}>
        <td class="td-center">
          {{ transaction[1]["created_at"]
             |inno_iso_to_date|date:"Y-m-d H:i T":m.kazoo.get_user_timezone }}
        </td>
        <td class="td-center">
          {% include "_currency_code_to_sign.tpl" transaction=transaction[1] %}
        </td>
        <td class="td-center">{{ transaction[1]["status"] }}</td>
        <td class="td-center">
          {% include "_set_payment_system_sign.tpl"
                     card_type=transaction[1]["card"][1]["card_type"]
          %}
          ***{{ transaction[1]["card"][1]["last_four"] }}
        </td>
      </tr>
      {% if transaction[1]["subscription_id"] %}
        {% wire id=transaction[1]["id"]
                action={dialog_open title=_"Transaction details"
